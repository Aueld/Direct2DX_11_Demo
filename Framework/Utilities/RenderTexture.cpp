#include "Framework.h"
#include "RenderTexture.h"

RenderTexture::RenderTexture()
{
	D3D11_TEXTURE2D_DESC textureDesc;
	ZeroMemory(&textureDesc, sizeof(D3D11_TEXTURE2D_DESC));

	// DESC 초기화
	{
		// 가로 세로 크기
		textureDesc.Width = WinMaxWidth;
		textureDesc.Height = WinMaxHeight;
		// 텍스처 형식과 사용 용도
		textureDesc.Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
		textureDesc.Usage = D3D11_USAGE_DEFAULT;
		// 어떤 리소스에 바인딩할지 지정
		textureDesc.BindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE;
		// 멀티 샘플링을 지원할 경우, 샘플링에 대한 정보를 지정
		textureDesc.SampleDesc.Count = 1;
		textureDesc.SampleDesc.Quality = 0;
		// 밉멥 레벨 수를 지정 (밉멥 : 하나의 텍스처 이미지를 작은 해상도로 만들어 복사하는 작업)
		textureDesc.MipLevels = 1;
		// 텍스처 배열 크기 (1로 설정하면 단일 텍스처)
		textureDesc.ArraySize = 1;
		// 기타 텍스처 옵션
		textureDesc.MiscFlags = 0;
	}

	HRESULT hr = DEVICE->CreateTexture2D(&textureDesc, nullptr, &texture);
	CHECK(hr);


	// Render Target View
	D3D11_RENDER_TARGET_VIEW_DESC rtvDesc;
	ZeroMemory(&rtvDesc, sizeof(D3D11_RENDER_TARGET_VIEW_DESC));

	// Desc 설정
	{
		rtvDesc.Format = textureDesc.Format;
		rtvDesc.ViewDimension = D3D11_RTV_DIMENSION_TEXTURE2D;
		rtvDesc.Texture2D.MipSlice = 0;
	}

	// RTV 생성
	hr = DEVICE->CreateRenderTargetView(texture, &rtvDesc, &rtv);
	CHECK(hr);


	// Shader Resource View
	D3D11_SHADER_RESOURCE_VIEW_DESC srvDesc;
	ZeroMemory(&srvDesc, sizeof(D3D11_SHADER_RESOURCE_VIEW_DESC));

	// DESC 설정
	{
		srvDesc.Format = textureDesc.Format;
		srvDesc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE2D;
		srvDesc.Texture2D.MipLevels = 1;
	}

	// SRV 생성
	hr = DEVICE->CreateShaderResourceView(texture, &srvDesc, &srv);
	CHECK(hr);
}

RenderTexture::~RenderTexture()
{
	SAFE_RELEASE(rtv);
	SAFE_RELEASE(texture);
}

void RenderTexture::RenderToTexture()
{
	// Om 단계에서 사용할 RTV 및 깊이-스텐실 뷰 설정
	DC->OMSetRenderTargets(1, &rtv, nullptr);
	DC->ClearRenderTargetView(rtv, ClearColor);
}

/*
Texture2D
- 그래픽 카드에서 사용하는 텍스처 데이터를 저장하는 객체
- 주로 이미지, 비디오, 노이즈, 라이트맵등을 저장
- 크기, 포맷, 밉맵, 멀티 샘플링 등 다양한 속성을 가질 수 있다

Shader Resource View
- 텍스처를 세이더에서 읽기 위한 뷰
- 세이더는 텍스처를 읽어서 픽셀값 등을 계산
- SRV를 사용하여 세이더가 텍스처 데이터에 엑세스 할 수 있다

Render Target View
- 렌더 타겟에 대한 뷰
- 세이더 결과를 저장하는 버퍼
- 해당 버퍼에 대한 읽기/쓰기 엑세스를 제공
- 셰이더 결과를 렌더 타겟에 저장할 수 있다

view
- 리소스의 특정 부분이나 특정 방식으로 해석된 데이터를 참조하는 객체

MipMap
- DirectX에서 아래의 목적을 위해서 텍스처의 여러 해상도 버전을 포함하는 이미지 컬렉션으로, 각각의 해상도 버전은 배열로 저장
- 텍스처 매핑이 더욱 효율적으로 이루어짐
	- 3D 공간에서 객체가 화면에 가까울수록 크게 보이고, 멀리 있을수록 작게 보인다
	- 가까운 객체에는 높은 해상도의 텍스처를 매핑하고, 멀리 있는 객체에는 낮은 해상도의 텍스처를 매핑한다
		-> 자원 사용량이 줄어들고 렌더링 속도가 빨라짐
*/